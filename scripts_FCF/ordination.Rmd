...
```{r}
otu_mat <- as.matrix(read.table(file = paste(save_location, 'OTU_counts_clean.tsv', sep = ""), sep = '\t', header = TRUE, row.names = 1))
tax_mat <- as.matrix(read.table(file = paste(save_location, 'taxonomy_table_clean.tsv', sep = ""), sep = '\t', header = TRUE, row.names = 1))
sample_meta <- read.table(file = paste(save_location, 'sample_metadata.tsv', sep = ""), header = TRUE, sep = '\t', row.names = 1)
...
```{r}
(clusters <- ward_OTU %>% set("labels_col", value = c("initial", "mid", "late"), k=3) %>% labels_colors)
