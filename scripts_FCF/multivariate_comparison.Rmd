...
phyloseq::plot_bar(ps, fill = "class") +
  ...
  labs(x = "", y = "Counts\n") +
  facet_wrap(~factor(focused.cluster, levels=c("early", "perturbed G", "perturbed M", "mid C", "mid G", "mid M", "late C", "late G", "late M")), scales = "free", ncol=3) +
...
```{r}
rows <- drive_permanova_on_cluster(phyloseq::subset_samples(ps, cluster == "initial" | cluster == "mid"), "i_vs_m")
PERMANOVA_results <- rbind(PERMANOVA_results, rows)
```
```{r}
rows <- drive_permanova_on_cluster(phyloseq::subset_samples(ps, cluster == "initial" | cluster == "late"), "i_vs_l")
PERMANOVA_results <- rbind(PERMANOVA_results, rows)
```
```{r}
...
```{r}
rows <- drive_permanova_on_focused_cluster(phyloseq::subset_samples(ps, cluster == "initial"), "i_focused.cluster")
PERMANOVA_results <- rbind(PERMANOVA_results, rows)
```
```{r}
...
```{r}
focused.cluster_levels <- c("early", "perturbed G", "perturbed M",
                          "mid C", "mid G", "mid M",
                          "late C", "late G", "late M")

focused.cluster_colors <- c("gray32", "slateblue1", "sienna1",
                          "springgreen3", "slateblue3", "sienna3", 
                          "springgreen4", "slateblue4", "sienna4")

focused.cluster_shapes <- c(0, 0, 0, 12, 12, 12, 15, 15, 15)
```
...
```{r}
# order factors
dispr$group <- factor(dispr$group, 
                      levels = focused.cluster_levels)

boxplot(dispr, main = "", xlab = "")
stripchart(dispr$distances ~ dispr$group, 
           vertical = TRUE, 
           method = "jitter",
           add = TRUE, 
           pch = focused.cluster_shapes,
