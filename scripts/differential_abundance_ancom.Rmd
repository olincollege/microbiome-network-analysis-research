...
ancom_on_cluster <- function(ps, test_id) {
  ...
  sig_ancom$ancom_relative_to <- treatment
...
drive_ancom_on_cluster <- function(ps, test_id) {
  ...
  rows <- ancom_on_cluster(ps, paste(test_id, "OTU-level", sep="_"))
  if (nrow(rows) > 0) {
...
drive_ancom_on_focused_cluster <- function(ps, test_id) {
  ...
  # perform once at OTU level
  rows <- ancom_on_focused_cluster(ps, paste(test_id, "OTU-level", sep="_"))
  if (nrow(rows) > 0) {
    rows$taxrank <- "OTU-level"
    results <- rbind(results, rows) 
  }
...
```{r}
# save to csv
write.csv(ANCOM_results, "../data/processed/ANCOM_results.csv", quote = FALSE)
```
...
ANCOM_results %>%
  ...
  write.csv("../data/processed/ANCOM_results_TRUE.csv", quote = FALSE)
```
